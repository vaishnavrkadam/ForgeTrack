...
export const Sidebar: React.FC = () => {
  const { activeTab, setActiveTab, issues, selectedProject } = useStore();

  const projectOpenIssuesCount = issues.filter(
    i => i.key.startsWith(selectedProject.key) && i.statusCategory !== 'DONE',
  ).length;
  ...
  const navItems = [
    ...
    { id: 'settings', label: 'Settings & Imports', icon: SettingsIcon },
  ];
